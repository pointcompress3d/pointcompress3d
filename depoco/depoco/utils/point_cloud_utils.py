...
def loadCloudFromBinary(file, cols=3):
    f = open(file, "rb")
    # Read the first few bytes to determine if it's a binary file
    header = f.read(100)
    f.seek(0)  # Reset file pointer

    if b"DATA binary" in header:
        # Binary PCD file
        binary_data = f.read()
        ...
        f = open(file, "r")
        # Assume it's a text PCD file
        lines = f.readlines()
        data_start = lines.index('DATA ascii\n') + 1
        data_lines = [line.strip().split() for line in lines[data_start:]]

        data = np.array(data_lines, dtype='float32')
        return data[:, :3] # Take only x,y,z for now
