...
class SubMapDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        out_dict['filename'] = self.submaps[index].getFileName()
        out_dict['normalizer'] = {'dif': self.submaps[index].normalizer.dif, 'min': self.submaps[index].normalizer.min}
        return out_dict
...
def createSubmaps(folders, nr_submaps=0, cols=3, on_the_fly=False,grid_size=40):
    ...
    for folder in sorted(folders):
        ...
        bin_files = sorted(glob.glob(folder+'*bin'))
        pcd_files = sorted(glob.glob(os.path.join(folder, '*pcd')))
        submap_files = bin_files + pcd_files
    if int(nr_submaps) != 0:
